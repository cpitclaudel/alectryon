...
```
$ alectryon references_typ.typ # Typst → JSON; produces ‘references_typ.alectryon.json’
$ typst compile --root . references_typ.typ references_typ.pdf # Typst → PDF; produces ‘references_typ.pdf’
```
...
#let jref = mref.with(counter-style: "_ i ro ha ni ho he to chi ri nu ru wo wa ka yo ta re so tsu ne na ra mu u wi no o ku ya ma ke fu ko e te a sa ki yu me mi shi we hi mo se su")
...
#mq-pr(".h{Permutation l l'}", block: true)
#mq-pr(".h{Permutation l' l''}", block: true)
...
#mquote(".io#scheme.s(Extraction Nat.add).msg(add)", language: "scheme", block: true)
...
```{coq}
(* -.h#l* -.h#[aA] -.s(Check let).msg(let) -.s{Proof.} *)
...
#massert(".io#plus.s(Print).msg{*Notation*}")
#massert(".s(Print).msg{*Nat.add*=*fix add*}")
